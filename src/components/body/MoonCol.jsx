        ...
        <div key={index} className="flex w-full">
          ...
          <div className="flex-grow flex justify-between">
            {dates.map((date, dateIndex) => {
              const count = getBookingForDate(floor.rooms[0], date).count;
              return (
                <div key={dateIndex} className="flex-1 p-2 border bg-[#F4F4F4] text-center">
                  <p>{count}</p>
                </div>
              );
            })}
            ...
            {floor.rooms.map((room, roomIndex) => (
               <div className="flex w-full">
                ...
                {dates.map((date, dateIndex) => {
                  ...
                  return (
                    <div key={dateIndex} className="flex-1 p-2 border bg-white relative">
                      {booking.occupied ? (
                        <BookingCard booking={booking} />
                      ) : (
                        <div className="border border-[#E1E1E1]">空房</div>
                      )}
                    </div>
                  );
                })}
